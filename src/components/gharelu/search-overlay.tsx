  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={cn("fixed top-0 left-0 w-full h-[60vh] p-0 border-none flex flex-col rounded-none shadow-2xl z-[100]", isNight ? "bg-[#0a110d] text-white" : "bg-[#FDFBF7] text-foreground")}>
        <div className={cn("p-6 pt-8", isNight ? "bg-black/40 border-b border-white/10" : "bg-primary border-b border-white/10")}>
          <DialogTitle className="text-white text-xl font-headline font-black mb-4">{isHindi ? 'बीमारी या नुस्खा खोजें' : 'Search Remedy or Illness'}</DialogTitle>
          ...
          <div className="p-4 max-w-2xl mx-auto">
            {results.length > 0 ? (
              <div className="space-y-2">
                {results.map((remedy) => (
                  <button key={remedy.id} onClick={() => handleResultClick(remedy)} className={cn("w-full p-4 rounded-xl border flex items-center gap-4 group text-left", isNight ? "bg-white/5 border-white/5" : "bg-white border-primary/5")}>
                    ...
                    <div className="flex-1 truncate">
                      <h4 className="font-bold text-base">{highlightMatchText(remedy.name[lang], query)}</h4>
                      <p className="text-[11px] opacity-60 truncate">{highlightMatchText(remedy.introduction[lang], query)}</p>
                    </div>
              ...
              <div className="text-center py-10 opacity-40 italic">{isHindi ? 'कोई परिणाम नहीं मिला' : 'No results found'}</div>
            ) : null}
          </div>
        </ScrollArea>
      </DialogContent>
